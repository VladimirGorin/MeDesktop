...
import time
import random 
from config.data import user_account_counter, user_send_message_direct, user_send_message_direct_text, nubmer, nubmerREGION, account_parse, account_channel
...
class TelegramBot():
    ...
    def check_exists_by_class_name(self, className):
        ...
        try:
            ...
            return False
        return True
    def send_message(self, user_send_message_direct_text, user_send_message_direct):
        browser = self.browser 

        for key, user_data in user_send_message_direct.items(): 
            for user in user_data:
                if self.check_exists_by_class_name(getInput):
                    ...
                    if(self.check_exists_by_xpath(getGlobalChat)):
                        ...
                        print("Пользователь не был найден в телеграмм")
                        return


                    time.sleep(6)

                    browser.find_element(By.CLASS_NAME, getInputMessage).send_keys(user_send_message_direct_text)
                    ...
                    with open("./info/send_message_the_user.txt", "a", encoding="utf-8") as file:
                        file.write(f"Сообщение было отправлено! > {user} \n")
                        file.close()
                else:
                    print(f"Этот аккуант был пропущен по причине - nothing coockie {Newnubmers}")
                    return
...
for key, value in user_account_counter.items():
    ...
    for item, itemV in user.items():
        Newnubmers = itemV
        my_bot = TelegramBot(user_send_message_direct_text, user_send_message_direct, Newnubmers)
        login(browser=my_bot.browser)
        my_bot.send_message(user_send_message_direct_text, user_send_message_direct)
