...
var iconStart = document.querySelector('#start1');
var iconMute = document.querySelector('#mute1');
...
var iconVideoStart = document.querySelector('#startvideo1');
var iconVideoMute = document.querySelector('#mutevideo1');
...
navigator.mediaDevices.getUserMedia({ audio: true})
    .then(stream => {
        ...
        document.querySelector('#start1').addEventListener('click', function(){
            socketAudio.send({'type': 'resetIcons', 'resetIcons': 'audio', 'on': '#mute1', 'off': '#start1', 'fightId': fightId})
        ...
        document.querySelector('#mute1').addEventListener('click', function(){
            socketAudio.send({'type': 'resetIcons', 'resetIcons': 'audio', 'on': '#start1', 'off': '#mute1', 'fightId': fightId})
...
navigator.mediaDevices.getUserMedia({ video: true})
    .then(stream => {
        ...
        let video = [];
        document.querySelector('#startvideo1').addEventListener('click', function(){
            socketAudio.send({'type': 'resetIcons', 'resetIcons': 'video', 'on': '#mutevideo1', 'off': '#startvideo1', 'fightId': fightId})
            console.log("Запись началась!");
        ...
        document.querySelector('#mutevideo1').addEventListener('click', function(){
          socketAudio.send({'type': 'resetIcons', 'resetIcons': 'video', 'on': '#startvideo1', 'off': '#mutevideo1', 'fightId': fightId})
          console.log("Запись приостановлена!");
          mediaRecorderV.stop();
        });
